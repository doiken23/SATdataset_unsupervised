...
from autoencoders import Stacked_AutoEncoder
...
def get_argument():
    ...
    parser.add_argument('--epochs', type=int, default=400, help='number of the epoch to train (default:400)')
    parser.add_argument('--lr', type=float, default=0.1, help='initial learning rate for training (default:0.01)')
    ...
    parser.add_argument('--milestones', type=list, default=[80, 160, 240, 320], help='epoch that reduce the learning rate (default:[every 80s])')
    parser.add_argument('data_path', type=str, help='the path of dataset (mat)')
    parser.add_argument('embedding_dimension', type=int, default=10, help='dimension of embedded feature (default:10)')
...
def main(args):
    ...
    val_loader    = data_utils.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2)
    ...
    net = Stacked_AutoEncoder(c * w * h, args.embedding_dimension)
    net.cuda()
    ...
    # Pretraining
    for stack_num, stack in enumerate(net.stacks):
        print("pre-training the stack {}".format(stack_num))

        # make pre-training layer list
        pretrain_layers = [net.layers[stack_num].parameters(), net.layers[-1 * (stack_num+1)].parameters()]

        # Define a Loss function and optimizer
        criterion = nn.MSELoss().cuda()
        optimizer = optim.SGD([
                                {'params': pretrain_layers[0]},
                                {'params': pretrain_layers[1]},
